...
export function useChatSocketHandlers({
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (data: Message) => {
      ...
      const isCurrentConversation = selectedUser?._id === (groupId || receiverId || senderId);
...
useEffect(() => {
  ...
  const handleGroupCreated = (response: any) => {
    ...
    const group: GroupChat = {
      ...
      createdBy: typeof response.createdBy === 'object' ? response.createdBy._id : response.createdBy,
      isGroup: true,
      lastMessage: null,
      unreadCount: 0
    };
    ...
    setUsers(prev => {
      ...
      if (exists) {
        ...
        // Update existing group instead of skipping
        return prev.map(u => u._id === group._id ? {
          ...group,
          participantsStatus: group.participants.reduce((acc, participant) => {
            acc[participant] = participant === group.createdBy ? 'joined' : 'pending';
            return acc;
      ...
      // Only add group if current user is a participant
      if (!group.participants.includes(myUserId)) {
        console.log('User not in group participants, skipping:', group._id);
        return prev;
      }
      
      // Initialize participants status if not provided
  ...
  const handleUsersAddedToGroup = (data: {
    ...
    // Check if current user is in the participants list (newly added)
    const isCurrentUserAdded = data.participants.includes(myUserId);
    ...
    setUsers(prev => {
      ...
      if (existingGroupIndex !== -1) {
        ...
        return prev.map(user => {
          if (user._id === data.conversationId && user.isGroup) {
            ...
            // Find newly added participants (compare with current participants)
            ...
            // Set status for newly added participants
            newlyAddedParticipants.forEach(participantId => {
              newParticipantsStatus[participantId] = 'joined';
            });
        ...
        // Group doesn't exist and current user was added - add the group to their list
        const newGroup = {
          _id: data.conversationId,
          name: data.groupName,
          avatar: data.groupAvatar || undefined,
          participants: data.participants,
          participantsStatus: data.participants.reduce((acc: any, participant: string) => {
            acc[participant] = participant === data.createdBy ? 'joined' : 'joined'; // All added users are joined
            return acc;
          }, {}),
      ...
      // Group doesn't exist and current user wasn't added - no change
      return prev;
    });

    // Find the admin/creator name from users list
    const adminUser = users.find(u => u._id === data.createdBy);
    const adminName = adminUser?.name || 'Admin';

    // Find newly added participants and their names
    ...
    // Call callback for toast notification and modal closure
    // Transform data to match expected callback format
    onUsersAddedToGroup?.({
  ...
  useEffect(() => {
    ...
    const handleMessagePinned = (data: { messageId: string; message?: Message }) => {
      ...
      setPinnedMessages(prev => {
        const messageToPin = messages.find(m => m._id === data.messageId) || data.message;
        return messageToPin && !prev.some(m => m._id === data.messageId)
          ? [...prev, { ...messageToPin, isPinned: true }]
          : prev;
      });
    };
  ...
  }, [socket, setMessages, setPinnedMessages, messages]);
